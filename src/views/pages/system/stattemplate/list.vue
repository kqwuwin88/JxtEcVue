        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            ...
            <div class="table-wrap" flex-box="1" v-if="chartType === '2'">
                <el-table :data="tableData" v-if="refresh" class="table-border" ref="multipleTable" height="100%" :summary-method="getSummaries" show-summary>
                    <el-table-column prop="tempId" label="模板" width=""></el-table-column>
...
export default {
	...
	methods:{
        ...
        exoprtList(){
            window.location.href=process.env.VUE_APP_APR + '/downloadSmsStatist?downType=3';
        },
        searchList(){
            if(this.chartType === '1'){
                this.queryChartData()
        ...
        queryChartData(){
            this.$api.statistOfTemplate({...this.formInline}).then( res => {
                this.barData = {
                    x: res.info.map(item=>item.tempId),
		...
		queryList(){
            this.tableData = []
            this.refresh = false
			this.$api.statistOfTemplate({...this.formInline,page:this.page}).then( res => { 
