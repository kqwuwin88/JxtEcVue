        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            ...
            <div class="table-wrap" flex-box="1" v-if="chartType === '2'">
                <el-table :data="tableData" v-if="refresh" class="table-border" ref="multipleTable" height="100%" :summary-method="getSummaries" show-summary>
                    <el-table-column prop="statistTime" label="时段" width=""></el-table-column>
...
export default {
	...
	methods:{
        ...
        exoprtList(){
            window.location.href=process.env.VUE_APP_APR + '/downloadSmsStatist?downType=2';
        },
        searchList(){
            if(this.chartType === '1'){
                this.queryChartData()
        ...
        queryChartData(){
            this.$api.statistOfHour({...this.formInline}).then( res => {
                this.barData = {
                    x: res.info.map(item=>item.statistTime),
		...
		queryList(){
            this.tableData = []
            this.refresh = false
			this.$api.statistOfHour({...this.formInline,page:this.page}).then( res => { 
