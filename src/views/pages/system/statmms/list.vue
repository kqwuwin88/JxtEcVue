<template>
    <div class="init-content-lay" flex="dir:top">
        ...
        <div class="content-list-wrap" flex="dir:top" flex-box="1">
            <div flex-box="0" style="color:#666666;" class="vsd-mb-10">注：由于手机用户停、关机等原因导致暂时未能送达的彩信，系统会在72小时内持续发送，因此当日的短信发送量仅供参考，以3天后的数据为准。</div>
            <div flex-box="1" v-if="chartType === '1'">
                <chartBar :barData="barData" title="彩信总数" />
            </div>
            <div class="table-wrap" flex-box="1" v-if="chartType === '2'">
                <el-table :data="tableData" v-if="refresh" class="table-border" ref="multipleTable" height="100%" :summary-method="getSummaries" show-summary>
                    ...
                    <el-table-column prop="smsNum" label="彩信总数" width=""></el-table-column>
                    <el-table-column prop="smsSucc" label="成功数" width=""></el-table-column>
                    <el-table-column prop="smsFail" label="失败数" width=""></el-table-column>
                    <el-table-column prop="succRate" label="成功率" width=""></el-table-column>
                </el-table>
            </div>
...
export default {
	...
	methods:{
        ...
        exoprtList(){
            window.location.href=process.env.VUE_APP_APR + '/downloadMmsStatist';
        },
        searchList(){
            if(this.chartType === '1'){
                this.queryChartData()
        ...
        queryChartData(){
            this.$api.statistMmsOfDate({...this.formInline}).then( res => {
                this.barData = {
                    x: res.info.map(item=>item.statistDate),
                    y1: res.info.map(item=>item.smsNum),
                    y2: res.info.map(item=>item.smsSucc),
                }
			}).catch(error=>{
				console.log(error)
			}) 
		...
		queryList(){
            this.tableData = []
            this.refresh = false
			this.$api.statistMmsOfDate({...this.formInline,page:this.page}).then( res => { 
